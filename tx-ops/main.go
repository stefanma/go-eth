...
// 2. 轮询跟踪：--tx <hash> --watch - 轮询跟踪交易状态直至确认
// 3. 发送交易：--send --to <address> --amount <eth> - 发起 ETH 转账交易
func main() {
	txHashHex := flag.String("tx", "", "transaction hash (for query mode)")
	watchMode := flag.Bool("watch", false, "poll until transaction is confirmed")
	pollInterval := flag.Duration("poll-interval", 3*time.Second, "interval between polls (for --watch)")
	pollTimeout := flag.Duration("poll-timeout", 5*time.Minute, "max time to wait for confirmation (for --watch)")
	...
	if *sendMode {
		...
		if *watchMode {
			watchTransaction(*txHashHex, *pollInterval, *pollTimeout)
		} else {
...
// watchTransaction 轮询跟踪交易状态，直至确认或超时
func watchTransaction(txHashHex string, interval, timeout time.Duration) {
	rpcURL := os.Getenv("ETH_RPC_URL")
	if rpcURL == "" {
		log.Fatal("ETH_RPC_URL is not set")
	}
	...
	txHash := common.HexToHash(trim0x(txHashHex))
	...
	pollOnce := func() (tx *types.Transaction, isPending bool, err error) {
	...
	for {
		...
		tx, isPending, err := pollOnce()
		if err != nil {
			log.Printf("[poll #%d] failed to get transaction: %v", pollCount, err)
			select {
			case <-ctx.Done():
				log.Fatalf("timeout after %v: transaction still pending", timeout)
			case <-ticker.C:
				continue
			}
		}
		...
		if isPending {
			fmt.Printf("[%s] poll #%d - Pending (hash: %s)\n", elapsed, pollCount, txHash.Hex())
			select {
			case <-ctx.Done():
				log.Fatalf("timeout after %v: transaction still pending", timeout)
			case <-ticker.C:
		...
		receipt, err := client.TransactionReceipt(ctx, txHash)
		if err != nil {
			log.Printf("transaction confirmed but failed to get receipt: %v", err)
			return
		}

		fmt.Printf("\n[%s] Transaction confirmed after %d polls\n\n", elapsed, pollCount)
		fmt.Println("=== Transaction ===")
		printTxBasicInfo(tx, false)
		fmt.Println("=== Receipt ===")
		printReceiptInfo(receipt)
		if receipt.Status == 1 {
			fmt.Println("\n✓ Status: Success")
